...
export default function StepByStepStudio({ onComplete }: StepByStepStudioProps) {
  ...
  const generateWithDirectAPI = async (_modelImageUrl: string, _garmentImageUrl: string): Promise<string> => {
    if (!user) throw new Error('User not authenticated');

    // For direct API, we'll simulate the process without actual image download
    // In production, you would implement the actual Gemini 2.5 Flash API here
    console.log('Starting Direct API generation...');
    console.log('Model URL:', _modelImageUrl);
    console.log('Garment URL:', _garmentImageUrl);

    // PLACEHOLDER: Direct API implementation
    // This simulates the direct API call without actually calling external APIs
    console.log('Simulating Direct API generation...');
    
    // Simulate processing time
    await new Promise(resolve => setTimeout(resolve, 3000));
    
    // Create a simple blob to simulate generated content
    // In production, this would be the actual image generated by Gemini 2.5 Flash
    ...
    if (ctx) {
      ...
      ctx.fillText('Direct API Generated', 200, 180);
      ctx.fillText('Image Placeholder', 200, 220);
    }
    
    // Convert canvas to blob
    const generatedBlob = await new Promise<Blob>((resolve) => {
      canvas.toBlob((blob) => resolve(blob!), 'image/png');
    });

    // Upload the generated image to Firebase Storage
    const timestamp = Date.now();
    const fileName = `api-generated-${timestamp}.png`;
    const storageRef = ref(storage, `generated/${user.uid}/${fileName}`);
    
    await uploadBytes(storageRef, generatedBlob);
  ...
  const renderStepIndicator = () => (
    <div className="flex justify-center items-center space-x-2 md:space-x-4 mb-4 md:mb-8">
      {steps.map((step, index) => {
        ...
        const _isPending = index > stepIndex;
